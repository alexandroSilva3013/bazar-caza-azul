...
type ProductStatus = "available" | "reserved" | "sold" | "unavailable";
...
interface Product {
  ...
  description: string; condition: string; price: number;
  status: ProductStatus; image: string; images?: string[];
}
...
const sampleProducts: Product[] = [
  { id:1, name:"Vestido Floral Feminino", category:"Feminino", description:"Lindo vestido floral em tecido leve e confortável, perfeito para o verão.", condition:"Seminovo - Ótimo estado", price:35, status:"available", image:"https://images.unsplash.com/photo-1595777457583-95e059d581b8?w=400&h=500&fit=crop" },
  { id:2, name:"Camisa Social Masculina", category:"Masculino", description:"Camisa social azul claro, ideal para trabalho e eventos formais.", condition:"Seminovo - Excelente estado", price:28, status:"available", image:"https://images.unsplash.com/photo-1596755094514-f87e34085b2c?w=400&h=500&fit=crop" },
  { id:3, name:"Conjunto Infantil Colorido", category:"Infantil", description:"Conjunto infantil com blusa e calça, tamanho 6 anos.", condition:"Novo com etiqueta", price:25, status:"reserved", image:"https://images.unsplash.com/photo-1519238263530-99bdd11df2ea?w=400&h=500&fit=crop" },
  { id:4, name:"Tênis Esportivo Nike", category:"Calçados", description:"Tênis Nike em excelente estado, numeração 40.", condition:"Seminovo - Pouco uso", price:65, status:"available", image:"https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=400&h=500&fit=crop" },
  { id:5, name:"O Pequeno Príncipe", category:"Livros", description:"Livro clássico em ótimo estado de conservação.", condition:"Seminovo - Muito bom", price:15, status:"available", image:"https://images.unsplash.com/photo-1544947950-fa07a98d237f?w=400&h=500&fit=crop" },
  { id:6, name:"Quebra-cabeça 500 Peças", category:"Brinquedos", description:"Quebra-cabeça completo com todas as peças.", condition:"Seminovo - Completo", price:20, status:"available", image:"https://images.unsplash.com/photo-1587563871167-1ee9c731aefb?w=400&h=500&fit=crop" },
  { id:7, name:"Bolsa Feminina Couro", category:"Acessórios", description:"Bolsa de couro sintético em excelente estado.", condition:"Seminovo - Ótimo", price:40, status:"available", image:"https://images.unsplash.com/photo-1548036328-c9fa89d128fa?w=400&h=500&fit=crop" },
  { id:8, name:"Jaqueta Jeans Feminina", category:"Feminino", description:"Jaqueta jeans versátil para diversas ocasiões.", condition:"Seminovo - Bom estado", price:45, status:"available", image:"https://images.unsplash.com/photo-1551028719-00167b16eac5?w=400&h=500&fit=crop" },
];
...
export default function App() {
  ...
  const filteredProducts = allProducts.filter(p => {
    ...
    const matchQ = p.name.toLowerCase().includes(searchQuery.toLowerCase()) || p.category.toLowerCase().includes(searchQuery.toLowerCase());
    const matchP = p.price >= priceRange[0] && p.price <= priceRange[1];
  ...
  const handleAdminLogin = (email: string, password: string) => {
    if (email === ADMIN_EMAIL && password === ADMIN_PASSWORD) {
      setIsAdminLoggedIn(true); setCurrentScreen("admin"); return true;
    }
    return false;
  };
  ...
  const addToCart = (product: Product) => {
    ...
    setCart(prev => {
      if (prev.find(i => i.product.id === product.id)) {
        toast.info(`${product.name} já está no carrinho!`); return prev;
      }
      toast.success(`${product.name} adicionado!`, { description: `R$ ${product.price.toFixed(2)}` });
  ...
  const StatusBadge = ({ status }: { status: ProductStatus }) => {
    const m: Record<ProductStatus,{cls:string;label:string}> = {
      available:{cls:"bg-green-100 text-green-700 border-green-200",label:"Disponível"},
  ...
  const ProductCard = ({ product }: { product: Product }) => {
    ...
    return (
      <div className="bg-white rounded-xl border border-border overflow-hidden hover:shadow-lg transition-all duration-300 group">
        <div className="aspect-[4/5] overflow-hidden bg-muted relative">
          <img src={product.image} alt={product.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
          <div className="absolute top-3 right-3"><StatusBadge status={product.status} /></div>
          {inCart && <div className="absolute top-3 left-3 bg-primary text-white text-xs px-2 py-1 rounded-full font-medium">No carrinho</div>}
        </div>
        <div className="p-5">
          <div className="text-xs text-muted-foreground mb-1.5 font-medium uppercase tracking-wide">{product.category}</div>
          <h3 className="font-semibold text-lg mb-2 line-clamp-1">{product.name}</h3>
          <span className="text-2xl font-bold text-primary block mb-4">R$ {product.price.toFixed(2)}</span>
          <div className="space-y-2">
            <button onClick={() => { setSelectedProduct(product); setCurrentScreen("product-detail"); }} className="w-full py-2 bg-primary text-white rounded-lg hover:bg-primary/90 transition-colors font-medium flex items-center justify-center gap-2">
              Ver Produto <ArrowRight className="w-4 h-4" />
            </button>
            {canAdd && (
              <button onClick={() => addToCart(product)} className={`w-full py-2 rounded-lg font-medium flex items-center justify-center gap-2 transition-colors border ${inCart ? "bg-green-50 text-green-700 border-green-200" : "bg-accent text-primary border-primary/20 hover:bg-primary/10"}`}>
      ...
      <div className="min-h-screen bg-gradient-to-br from-[#0b1a3b] via-[#163E8F] to-[#1a4aab] flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-md">
          ...
          <div className="bg-white rounded-2xl shadow-2xl p-8">
            ...
            <form onSubmit={(e) => { e.preventDefault(); if (!handleAdminLogin(email, password)) setError("E-mail ou senha incorretos."); }} className="space-y-5">
              <div>
            ...
            <div className="space-y-4">
              {userReservations.map(res => (
                <div key={res.id} className="bg-white rounded-2xl border border-border p-6 hover:shadow-md cursor-pointer" onClick={() => { setSelectedProduct(res.product); setCurrentScreen("product-detail"); }}>
                  <div className="flex items-center gap-5">
                    <div className="w-20 h-20 rounded-xl overflow-hidden bg-muted flex-shrink-0"><img src={res.product.image} alt={res.product.name} className="w-full h-full object-cover" /></div>
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                        <div><h3 className="font-semibold text-lg">{res.product.name}</h3><p className="text-muted-foreground text-sm">{res.product.category}</p>
                          <div className="flex items-center gap-4 mt-2"><span className="text-xl font-bold text-primary">R$ {res.product.price.toFixed(2)}</span><span className="text-xs text-muted-foreground flex items-center gap-1"><Calendar className="w-3.5 h-3.5" />{res.date}</span></div>
                        </div>
              ...
              <div>
                ...
                <div className="space-y-3">{selected.items.map((item,i)=>(
                  <div key={i} className="flex items-center gap-4 p-4 bg-muted rounded-xl">
                    <div className="w-14 h-14 rounded-lg overflow-hidden flex-shrink-0"><img src={item.product.image} alt={item.product.name} className="w-full h-full object-cover" /></div>
                    <div className="flex-1"><p className="font-medium">{item.product.name}</p><p className="text-sm text-muted-foreground">{item.product.category}</p></div>
                    <div className="text-right"><p className="text-xs text-muted-foreground">Qtd: {item.quantity}</p><p className="font-bold text-primary">R$ {(item.product.price*item.quantity).toFixed(2)}</p></div>
                  </div>
                ))}</div>
              </div>
          ...
          {orders.length === 0 ? (
            ...
            <div className="space-y-4">{orders.map(o=>(
              <div key={o.id} className="bg-white rounded-2xl border border-border p-6 hover:shadow-md cursor-pointer" onClick={() => setSelected(o)}>
                ...
                <div className="flex items-center gap-3 mb-4">
                  {o.items.slice(0,3).map((item,i)=>(<div key={i} className="w-12 h-12 rounded-lg overflow-hidden flex-shrink-0 bg-muted"><img src={item.product.image} alt={item.product.name} className="w-full h-full object-cover" /></div>))}
            ...
            <div className="lg:col-span-2 space-y-4">
              {cart.map(item=>(
                <div key={item.product.id} className="bg-white rounded-2xl border border-border p-5">
                  <div className="flex items-center gap-5">
                    <div className="w-20 h-20 rounded-xl overflow-hidden bg-muted flex-shrink-0"><img src={item.product.image} alt={item.product.name} className="w-full h-full object-cover" /></div>
                    <div className="flex-1 min-w-0">
                      <div className="text-xs text-muted-foreground uppercase tracking-wider mb-1">{item.product.category}</div>
                      <h3 className="font-semibold leading-tight">{item.product.name}</h3>
                      <p className="text-primary font-bold text-lg mt-1">R$ {item.product.price.toFixed(2)}</p>
                    </div>
                    <div className="flex flex-col items-end gap-3">
                      ...
                      <p className="text-sm text-muted-foreground">Subtotal: <span className="font-semibold text-foreground">R$ {(item.product.price*item.quantity).toFixed(2)}</span></p>
                    </div>
                  </div>
                </div>
              ))}
                ...
                <div className="space-y-3 mb-5">
                  {cart.map(item=>(
                    <div key={item.product.id} className="flex items-center justify-between text-sm">
                      ...
                      <span className="font-medium flex-shrink-0">R$ {(item.product.price*item.quantity).toFixed(2)}</span>
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-3">
                  {cart.map(item=>(
                    <div key={item.product.id} className="flex items-center gap-4 p-4 bg-muted rounded-xl">
                      <div className="w-14 h-14 rounded-xl overflow-hidden flex-shrink-0"><img src={item.product.image} alt={item.product.name} className="w-full h-full object-cover" /></div>
                      <div className="flex-1 min-w-0"><p className="font-medium line-clamp-1">{item.product.name}</p><p className="text-sm text-muted-foreground">{item.product.category}</p></div>
                      <div className="text-right flex-shrink-0"><p className="text-xs text-muted-foreground">Qtd: {item.quantity}</p><p className="font-bold text-primary">R$ {(item.product.price*item.quantity).toFixed(2)}</p></div>
  ...
  const OrderConfirmationScreen = () => {
    ...
    const lines = latest?.items.map(i => `- ${i.product.name} (${i.quantity}x) – R$ ${(i.product.price*i.quantity).toFixed(2)}`).join("\n") || "";
              ...
              <div className="bg-muted rounded-xl p-5 mb-6 text-left">
                ...
                <div className="space-y-2 mb-3">{latest.items.map((item,i)=>(
                  <div key={i} className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-lg overflow-hidden flex-shrink-0"><img src={item.product.image} alt={item.product.name} className="w-full h-full object-cover" /></div>
                    <span className="text-sm flex-1 line-clamp-1">{item.product.name}</span>
                    <span className="text-sm font-medium text-primary">R$ {item.product.price.toFixed(2)}</span>
                  </div>
                ))}</div>
            ...
            <div className="p-8 space-y-8">
              ...
              <div>
                ...
                <div className="flex items-center gap-5 p-5 bg-muted rounded-2xl">
                  <div className="w-24 h-24 rounded-xl overflow-hidden flex-shrink-0"><img src={selectedProduct.image} alt={selectedProduct.name} className="w-full h-full object-cover" /></div>
                  <div><div className="text-xs text-muted-foreground uppercase tracking-wider mb-1">{selectedProduct.category}</div><h3 className="font-semibold text-lg">{selectedProduct.name}</h3><p className="text-2xl font-bold text-primary mt-1">R$ {selectedProduct.price.toFixed(2)}</p></div>
                </div>
              </div>
  ...
  const ConfirmationScreen = () => {
    ...
    const phone = currentUser?.phone || "";
    const pName = selectedProduct?.name || "Produto";
          ...
          <div className="bg-white rounded-2xl border border-border p-10 text-center shadow-sm">
            ...
            {selectedProduct && (
              <div className="flex items-center gap-4 p-4 bg-muted rounded-xl mb-8 text-left">
                <div className="w-16 h-16 rounded-lg overflow-hidden flex-shrink-0"><img src={selectedProduct.image} alt={selectedProduct.name} className="w-full h-full object-cover" /></div>
                <div><p className="font-semibold">{selectedProduct.name}</p><p className="text-primary font-bold text-lg">R$ {selectedProduct.price.toFixed(2)}</p></div>
              </div>
            )}
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-xl border border-border p-6 sticky top-28 space-y-6">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {[{v:"all",l:"Todos"},{v:"available",l:"Disponível"},{v:"reserved",l:"Reservado"},{v:"unavailable",l:"Indisponível"}].map(o=>(
      ...
      <div className="min-h-screen bg-background"><Navbar />
        <div className="pt-28 pb-20"><div className="container mx-auto px-4 lg:px-8">
          ...
          <div className="grid lg:grid-cols-2 gap-12">
            <div>
              <div className="aspect-[4/5] rounded-2xl overflow-hidden bg-muted mb-4"><img src={selectedProduct.image} alt={selectedProduct.name} className="w-full h-full object-cover" /></div>
            </div>
            <div className="space-y-6">
              <div>
                <div className="text-sm text-muted-foreground mb-2 uppercase tracking-wide font-medium">{selectedProduct.category}</div>
                <h1 className="text-3xl md:text-4xl font-bold mb-4" style={{fontFamily:"Poppins,sans-serif"}}>{selectedProduct.name}</h1>
                <div className="flex items-center gap-4"><span className="text-4xl font-bold text-primary">R$ {selectedProduct.price.toFixed(2)}</span><StatusBadge status={selectedProduct.status} /></div>
              </div>
              <div className="border-t border-b border-border py-6 space-y-4">
                <div><h3 className="font-semibold mb-2">Descrição</h3><p className="text-muted-foreground leading-relaxed">{selectedProduct.description}</p></div>
                <div><h3 className="font-semibold mb-2">Estado de Conservação</h3><p className="text-muted-foreground">{selectedProduct.condition}</p></div>
              </div>
              ...
              {selectedProduct.status === "available" ? (
                ...
                <button disabled className="w-full py-4 bg-muted text-muted-foreground rounded-xl font-semibold text-lg cursor-not-allowed">{selectedProduct.status==="sold"?"Produto Vendido":"Produto Indisponível"}</button>
              )}
  ...
  const AdminScreen = () => {
    ...
    const [pForm, setPForm] = useState({name:"",category:"Feminino" as Category,description:"",condition:"",price:"",status:"available" as ProductStatus,image:""});

    const openAdd = () => { setEditingProduct(null); setPForm({name:"",category:"Feminino",description:"",condition:"",price:"",status:"available",image:""}); setShowModal(true); };
    const openEdit = (p: Product) => { setEditingProduct(p); setPForm({name:p.name,category:p.category,description:p.description,condition:p.condition,price:String(p.price),status:p.status,image:p.image}); setShowModal(true); };

    const saveProduct = () => {
      const price = parseFloat(pForm.price)||0;
      if (!pForm.name || !pForm.price) { toast.error("Preencha nome e valor"); return; }
      if (editingProduct) {
        setAllProducts(prev => prev.map(p => p.id===editingProduct.id ? {...p,...pForm,price,images:[pForm.image||p.image]} : p));
        toast.success("Produto atualizado!");
      } else {
        const id = Math.max(...allProducts.map(p=>p.id),0)+1;
        const img = pForm.image||"https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=400&h=500&fit=crop";
        setAllProducts(prev => [...prev, {id,...pForm,price,image:img,images:[img]}]);
        toast.success("Produto adicionado!");
      }
      setShowModal(false);
    };

    const deleteProd = (id: number) => { setAllProducts(p => p.filter(x => x.id!==id)); setDeleteId(null); toast.success("Produto removido."); };
    ...
    const pieData = [
      {name:"Disponível",value:allProducts.filter(p=>p.status==="available").length,color:"#22c55e"},
      {name:"Reservado",value:allProducts.filter(p=>p.status==="reserved").length,color:"#eab308"},
      {name:"Vendido",value:allProducts.filter(p=>p.status==="sold").length,color:"#6b7280"},
      {name:"Indisponível",value:allProducts.filter(p=>p.status==="unavailable").length,color:"#d1d5db"},
    ].filter(d=>d.value>0);
    const catData = CATEGORIES.map(cat=>({
      name:cat.length>8?cat.slice(0,7)+".":cat,
      Disponível:allProducts.filter(p=>p.category===cat&&p.status==="available").length,
      Reservado:allProducts.filter(p=>p.category===cat&&p.status==="reserved").length,
      Vendido:allProducts.filter(p=>p.category===cat&&p.status==="sold").length,
    }));
            ...
            <div>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                {[
                  ...
                  {I:Check,v:allProducts.filter(p=>p.status==="available").length,l:"Disponíveis",c:"text-green-600",b:"bg-green-100"},
                  {I:Clock,v:allProducts.filter(p=>p.status==="reserved").length,l:"Reservados",c:"text-yellow-600",b:"bg-yellow-100"},
                  {I:ShoppingBag,v:allProducts.filter(p=>p.status==="sold").length,l:"Vendidos",c:"text-gray-600",b:"bg-gray-200"},
                  ...
                  <table className="w-full">
                    ...
                    <tbody>{allProducts.map(p=>(
                      <tr key={p.id} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                        <td className="p-4"><div className="w-14 h-14 rounded-xl overflow-hidden bg-muted"><img src={p.image} alt={p.name} className="w-full h-full object-cover" /></div></td>
                        <td className="p-4 font-medium">{p.name}</td>
                        <td className="p-4 text-muted-foreground text-sm">{p.category}</td>
                        <td className="p-4 font-semibold text-primary">R$ {p.price.toFixed(2)}</td>
                ...
                {CATEGORIES.map((cat,i)=>(
                  <div key={i} className="bg-white rounded-2xl border border-border p-6 hover:shadow-md transition-shadow">
                    ...
                    <div className="flex items-center gap-4 text-sm">
                      <span className="text-muted-foreground">{allProducts.filter(p=>p.category===cat).length} produto(s)</span>
                      <span className="text-green-600">{allProducts.filter(p=>p.category===cat&&p.status==="available").length} disponíveis</span>
                    </div>
                  </div>
                ))}
              </div>
                  ...
                  <table className="w-full">
                    ...
                    <tbody>{localOrders.map(o=>(
                      <tr key={o.id} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                        ...
                        <td className="p-4 text-sm max-w-[160px] truncate">{o.items.map(i=>i.product.name).join(", ")}</td>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                {[
                  ...
                  {l:"Produtos Reservados",v:allProducts.filter(p=>p.status==="reserved").length,c:"text-yellow-600"},
          ...
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={e => { if (e.target===e.currentTarget) setShowModal(false); }}>
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="p-6 space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div><label className="block text-sm font-medium mb-2">Nome *</label><input type="text" value={pForm.name} onChange={e => setPForm(f=>({...f,name:e.target.value}))} placeholder="Nome do produto" className="w-full px-4 py-3 bg-muted rounded-xl border border-border focus:outline-none focus:ring-2 focus:ring-primary/20" /></div>
                  <div><label className="block text-sm font-medium mb-2">Categoria</label>
                    <select value={pForm.category} onChange={e => setPForm(f=>({...f,category:e.target.value as Category}))} className="w-full px-4 py-3 bg-muted rounded-xl border border-border focus:outline-none focus:ring-2 focus:ring-primary/20">
                      {CATEGORIES.map(c=><option key={c}>{c}</option>)}
                    </select>
                  </div>
                </div>
                <div><label className="block text-sm font-medium mb-2">Descrição</label><textarea value={pForm.description} onChange={e => setPForm(f=>({...f,description:e.target.value}))} rows={3} placeholder="Descrição..." className="w-full px-4 py-3 bg-muted rounded-xl border border-border focus:outline-none focus:ring-2 focus:ring-primary/20 resize-none" /></div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div><label className="block text-sm font-medium mb-2">Estado de Conservação</label><input type="text" value={pForm.condition} onChange={e => setPForm(f=>({...f,condition:e.target.value}))} placeholder="Ex: Seminovo - Ótimo" className="w-full px-4 py-3 bg-muted rounded-xl border border-border focus:outline-none focus:ring-2 focus:ring-primary/20" /></div>
                  <div><label className="block text-sm font-medium mb-2">Valor (R$) *</label><input type="number" step="0.01" min="0" value={pForm.price} onChange={e => setPForm(f=>({...f,price:e.target.value}))} placeholder="0,00" className="w-full px-4 py-3 bg-muted rounded-xl border border-border focus:outline-none focus:ring-2 focus:ring-primary/20" /></div>
                </div>
                <div><label className="block text-sm font-medium mb-2">Status</label>
                  <select value={pForm.status} onChange={e => setPForm(f=>({...f,status:e.target.value as ProductStatus}))} className="w-full px-4 py-3 bg-muted rounded-xl border border-border focus:outline-none focus:ring-2 focus:ring-primary/20">
                    <option value="available">Disponível</option><option value="reserved">Reservado</option><option value="sold">Vendido</option><option value="unavailable">Indisponível</option>
                  </select>
                </div>
                <div><label className="block text-sm font-medium mb-2">URL da Imagem</label>
                  <input type="url" value={pForm.image} onChange={e => setPForm(f=>({...f,image:e.target.value}))} placeholder="https://..." className="w-full px-4 py-3 bg-muted rounded-xl border border-border focus:outline-none focus:ring-2 focus:ring-primary/20" />
                  {pForm.image && <div className="mt-3 w-24 h-24 rounded-xl overflow-hidden bg-muted"><img src={pForm.image} alt="preview" className="w-full h-full object-cover" onError={e => {(e.target as HTMLImageElement).style.display="none";}} /></div>}
                </div>
              </div>
